...
class MiaoShouAssistant(object):
        ...
        with gr.TabItem('Boot Assistant', elem_id="boot_assistant_tab") as boot_assistant:
            with gr.Row():
                with gr.Column(elem_id="col_model_list"):
                    ...
                    with gr.Row():
                        with gr.Column():
                            txt_save_status = gr.Markdown(visible=False, show_label=False)
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        md_vram_release = gr.Markdown(visible=False, value='Memory Released', show_label=False)
                    with gr.Row():
    ...
    def save_cmdline_args(self, drp_gpu, drp_theme, txt_listen_port, chk_group_args, additional_args):
        ...
        #launch.start()
        return gr.Markdown(value="Settings Saved", visible=True)
    ...
    def switch_model_source(self, new_model_source: str, search, chk_nsfw, base_model, model_type, model_tag, sort_by):
        ...
        return (
            gr.Dataset(samples=images),
            gr.Button(visible=show_download_button),
            gr.HTML(visible=not show_download_button)
        )
