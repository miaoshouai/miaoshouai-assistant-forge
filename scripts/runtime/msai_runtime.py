...
class MiaoshouRuntime(object):
    ...
    def update_my_model_type(self, search_txt, model_type) -> t.Dict:
        ...
        return gr.Dataset(samples=my_models)
        ...
        for model in self.ds_my_models.samples:
            try:
                if model[0] == self.prelude.no_preview_img and model[1] != 0:
                    gallery, v1, l1, htmlDetail, h2, l2, img_list = self.get_model_info(model)
    ...
    def select_version(self, models, version_name):
        ...
        return (
            gr.Dropdown(choices=drop_list, value=drop_list[0] if len(drop_list) > 0 else []),
            cover_imgs,
            gr.HTML(value=f'<p style="text-align: center;">'
                     f'<a style="text-align: center;" href="{download_url_by_default}" '
                     'target="_blank">Download</a></p>')
                )

    def get_model_info(self, models):
        ...
        return (
            gr.Dataset(samples=cover_imgs),
            gr.Dropdown(choices=version_list, value=version_list[0] if len(version_list) > 0 else []),
            gr.Dropdown(choices=drop_list, value=drop_list[0] if len(drop_list) > 0 else []),
            htmlDetail,

            gr.HTML(value=f'<div class="lg secondary gradio-button svelte-cmf5ev" style="text-align: center;"><a style="text-align: center;" href="{download_url_by_default}" target="_blank">Download</a></div>'),
            gr.Dropdown(choices=sub_folder, value=sub_folder[0] if len(sub_folder) > 0 else []),
            cover_imgs
        )
    ...
    def get_my_model_covers(self, model, model_type):
        ...
        return gr.Dataset(samples=cover_list), gr.HTML(visible=True, value=new_html), gr.HTML(visible=True, value=button_html)
    ...
    def update_cover_info(self, model, covers):
        ...
        return gr.Button(visible=True), gr.Text(value=generation_info), gr.Image(value=fname)

    def get_downloading_status(self):
        (_, _, desc) = self.downloader_manager.tasks_summary()
        return gr.HTML(value=desc)
    ...
    def download_model(self, filename: str, des_folder: str):
        ...
        #self.model_files.clear()
        return gr.HTML(value=f"<h4>{len(urls)} downloading tasks added into task list</h4>")
    ...
    def change_model_folder(self, folder_path=''):
        ...
        if folder_path == '':
            return gr.Markdown(value='No directory is set', visible=True)
        ...
        return gr.Markdown(value=res, visible=True)
    ...
    def check_update(self):
        ...
        return gr.Markdown(visible=True, value=update_status), gr.Checkbox(visible=show_update), gr.Button(visible=show_update)
    ...
    '''def get_gpt_prompt(self, model, model_type, main_prompt):
        ...
        if model is None:
            return gr.TextArea(value='Please select a model first')
        ...
        try:
            ...
            res_prompt = str(e)
            return gr.TextArea(value=res_prompt)
    ...
    def update_gptapi(self, apikey):
        ...
        return gr.Markdown(value=res, visible=True), gr.Textbox(placeholder=gpt_hint_text, value=value_text)'''
